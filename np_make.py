...
def reserve_memory(path: str, size: int) -> tuple:
    ...
    rgb_images = np.zeros((total_num_files, size, size, 3), dtype=np.uint8)
    gray_images  = np.zeros((total_num_files, size, size, 3), dtype=np.uint8)
    return rgb_images, gray_images


def preprocess_1(image: np.ndarray, size: int) -> np.ndarray:
    image = cv2.cvtColor(src=image, code=cv2.COLOR_BGR2RGB)
    return cv2.resize(src=image, dsize=(size, size), interpolation=cv2.INTER_AREA)


def preprocess_2(image: np.ndarray, size: int) -> np.ndarray:
...
def main() -> None:
    ...
    rgb_images, gray_images = reserve_memory(path, size)
    ...
    for folder_name in folders:
        for filename in os.listdir(os.path.join(path, folder_name)):
            image_1 = cv2.imread(os.path.join(os.path.join(path, folder_name), filename), cv2.IMREAD_COLOR)
            image_2 = image_1.copy()

            rgb_images[i] = preprocess_1(image_1, size)
            gray_images[i]  = preprocess_2(image_2, size)
    ...
    if kaggle:
        ...
        np.save(os.path.join(path, "rgb_images.npy"), rgb_images)
        np.save(os.path.join(path, "gray_images.npy"), gray_images)
